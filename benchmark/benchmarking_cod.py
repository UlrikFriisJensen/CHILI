...
from Code.datasetClass_cod import InOrgMatDatasets
...
# Define your dataset
dataset = InOrgMatDatasets(dataset=config_dict['dataset'], root=config_dict['root'], processed_dir='Dataset/COD_subset_v4/processed')
try:
    if config_dict['dataset'] == 'COD_subset_v4':
        dataset.load_data_split(split_strategy='stratified', stratify_on='Crystal system (Number)', stratify_distribution='equal')
    else:
        dataset.load_data_split(split_strategy='random')
except FileNotFoundError:
    #print('Could not find datasplit', flush=True)
    if config_dict['dataset'] == 'COD_subset_v4':
        dataset.create_data_split(split_strategy='stratified', stratify_on='Crystal system (Number)', stratify_distribution='equal')
        dataset.load_data_split(split_strategy='stratified', stratify_on='Crystal system (Number)', stratify_distribution='equal')
    else:
...
for i, seed in enumerate(config_dict['Train_config']['seeds']):
    ...
    def rwp(
        ...
        return torch.mean(torch.sqrt(torch.sum(torch.square(pred - truth), dim=-1) / torch.sum(torch.square(truth), dim=-1)))

    # Define optimizer and loss function
    optimizer = torch.optim.Adam(list(model.parameters()) + list(secondary.parameters()), lr=config_dict['Train_config']['learning_rate'])
    if config_dict['task'] in ['AtomClassification', 'SpacegroupClassification', 'CrystalSystemClassification']:
        ...
        n_classes = 1
        criterion = torch.nn.SmoothL1Loss()#rwp
        metric = torch.nn.MSELoss()#rwp
    ...
    for epoch in range(config_dict['Train_config']['epochs']):
        ...
        for data in train_loader:
            batches_pbar.set_description('Sending to device', refresh=True)
            data = data.to(device)
            
            batches_pbar.set_description('Forward pass', refresh=True)
            if config_dict['task'] == 'AtomClassification':
            ...
            batches_pbar.set_description('Calulating loss', refresh=True)
            loss = criterion(out, ground_truth)
            batches_pbar.set_description('Zeroing gradients', refresh=True)
            optimizer.zero_grad()
            loss.backward()
            batches_pbar.set_description('Step', refresh=True)
            optimizer.step()
        ...
        #print('Finished validation loop', flush=True)
    ...
    #print('Finished testing loop', flush=True)
    ...
    #print('Finished Tensorboard logging', flush=True)
    ...
    #print('Added results to dataframe', flush=True)
