...
class InOrgMatDatasets(Dataset):
    def __init__(self, dataset, root='./', transform=None, pre_transform=None, pre_filter=None, processed_dir=None):
    ...
    def get_statistics(self, return_dataframe=False):
        ...
        if stat_path.exists():
            ...
            for idx in trange(self.len(split=None)):
        ...
        if return_dataframe:
            if self.train_set is not None:
                return df_stats.loc[list(self.train_set.indices) + list(self.validation_set.indices) + list(self.test_set.indices)].reset_index(drop=True)
            else:
                return df_stats
        else:
            return None
