...
class h5Constructor():
    ...
    def gen_single_h5(self, input_tuple, override=False, verbose=False, check_connectivity=False, check_periodic=False, save_discrete_nps=True):
        ...
        try:
            ...
            print(f'No access to spacegroup number in {cif_name}, setting to Unknown', flush=True)
            pass
        ...
        try:
            unit_cell_occ = [list(d.values())[0] for d in unit_cell.info['occupancy'].values()]
        ...
        try:
            ...
            if np.any(np.logical_and(unit_cell_distances < 1.2, unit_cell_distances > 0.0)):
        ...
        xray_calculator = DebyeCalculator(device=device, qmin=1, qmax=30, qstep=0.05, biso=0.3, rmin=0.0, rmax=60.0, rstep=0.01, radiation_type='xray')
        neutron_calculator = DebyeCalculator(device=device, qmin=1, qmax=30, qstep=0.05, biso=0.3, rmin=0.0, rmax=60.0, rstep=0.01, radiation_type='neutron')
    ...
    def gen_h5s(self, np_radii=[5., 10., 15., 20., 25.], parallelize=True, num_processes=cpu_count() - 1, device=None):
        ...
        inputs = zip(self.cifs, repeat(np_radii), repeat(device), repeat(node_feature_table), repeat(metals_numbers), repeat(metals_elements))
