...
from ase.spacegroup import (
    crystal,  # https://wiki.fysik.dtu.dk/ase/ase/spacegroup/spacegroup.html#ase.spacegroup.crystal
)
...
class structureGenerator:
    ...
    def calculate_cell_parameter_approximations(self):
        all_point_dict = dict()
        all_fit_dict = dict()
        for structure_type in tqdm(
            self.structure_types, desc="Calculating cell parameter approximations"
        ):
    ...
    def visualize_cell_parameter_fits(self, structure_type):
        ...
        sns.lineplot(
            x=x, y=x * fit_params[0] + fit_params[1], color="k", linewidth=2, ax=ax[0]
        )
        ax[0].annotate(
            f"y = {fit_params[0]:.1e} * x + {fit_params[1]:.2f}",
            xy=(0.05, 0.95),
        ...
        if n_params == 2:
            ...
            ax[1].set_title(
                f"Cellparameter {params[1]}", fontsize=14, fontweight="bold"
            )
            ax[1].annotate(
                f"y = {fit_params[0]:.1e} * x + {fit_params[1]:.2f}",
                xy=(0.05, 0.95),
    ...
    def estimate_cell_parameters(self, crystal_type, compound, hea_mean_radius=None):
        atom_list = [
            re.sub("[1-9]*|H[1-9]+", "", atom)
            for atom in re.findall("[A-Z][^A-Z]*", compound)
        ]
        if crystal_type in ["RockSalt", "ZincBlende", "CaesiumChloride"]:
            ...
            compound = "".join(
                [
                    atom + str(occurence) if occurence != 1 else atom
                    for atom, occurence in zip(atom_list, atom_occurence)
                ]
            )
    ...
    def infer_cell_parameters(
        ...
        if crystal_type == "SC":
            ...
            if hea_mean_radius:
                a = (4.0 / np.sqrt(3.0)) * hea_mean_radius / 100.0
            else:
                atom_info = element(compound)
            ...
            if hea_mean_radius:
                a = (8.0 * hea_mean_radius / 100.0) / np.sqrt(3.0)
            else:
                atom_info = element(compound)
            ...
            try:
                ...
                raise KeyError(
                    f"'{crystal_type}' does not match the keys in the lookup table. Available keys are: {list(self.parameter_lookup.keys())}"
                ) from err
            try:
                ...
                if len(dict_keys) == 1:
                    ...
                    key_index = int(
                        input("Input the index (0 indexing) of the compound to use:")
                    )
                    ...
                    if verbose:
                        print(
                            f"No match found for {atoms}.\nEstimating cell parameters from table values."
                        )
                    compound, cellparams = self.estimate_cell_parameters(
                        crystal_type, compound, hea_mean_radius=hea_mean_radius
                    )
        cellparams.update(alpha=90.0, gamma=120.0)
        return compound, cellparams
    ...
    def create_cell(
        ...
        if cellparams == "infer":
            compound, cellparams = self.infer_cell_parameters(
                crystal_type, atoms, hea_mean_radius=hea_mean_radius, verbose=verbose
            )
    ...
    def create_hea(
        ...
        hea_mean_radius = np.average(
            [self.get_radius(atom) for atom in atoms], weights=stoichiometry
        )
        ...
        hea_atoms = np.random.choice(
            atoms, size=(n_atoms,), p=stoichiometry, replace=True
        )
    ...
    def create_cif_dataset(
        ...
        for crystal_type in tqdm(crystal_types, desc="Structure types"):
            if (n_species == 2) and (
                crystal_type in ["Perovskite", "PotassiumTetrafluorideNickelate"]
            ):
                continue
            ...
            if from_table_values:
                for structure in self.parameter_lookup[crystal_type]:
                    atom_list = [
                        re.sub("[1-9]*|H[1-9]*", "", atom)
                        for atom in re.findall("[A-Z][^A-Z]*", structure)
                    ]
                    atom_list = np.unique(atom_list)
                    if required_atoms:
                        required_atoms_check = all(
                            req_atom in structure for req_atom in required_atoms
                        )
                    else:
                    ...
                    if required_atoms_check and number_of_species_check:
                        cif = self.create_cell(
                            crystal_type, structure, size=unit_cell_size
                        )
                        write(
                            f"{save_folder}{crystal_type}_{structure.split('_')[-1]}.cif",
                            format="cif",
                            images=cif,
                        )
            else:
                structure_list = [
                    ...
                    for atom_tuple in zip(
                        optional_atoms, repeat("".join(required_atoms))
                    )
                ]
                for structure in tqdm(structure_list, desc="Structures", leave=False):
                    ...
                    cif, structure = self.create_cell(
                        crystal_type, structure, size=unit_cell_size, return_name=True
                    )
                    write(
                        f"{save_folder}{crystal_type}_{structure.split('_')[-1]}.cif",
                        format="cif",
